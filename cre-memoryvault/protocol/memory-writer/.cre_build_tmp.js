...
function uint8ArrayToBase64(bytes) {
  ...
  for (let i2 = 0;i2 < bytes.length; i2 += 3) {
    ...
    result += i2 + 1 < bytes.length ? chars[(b & 15) << 2 | c >> 6] : "=";
    result += i2 + 2 < bytes.length ? chars[c & 63] : "=";
  }
  return result;
}
function encryptData(plaintext, keyHex) {
  ...
  const iv = hexToBytes3(ivHex.slice(2, 26));
  const ciphertext = new Uint8Array(encoded.length);
  for (let i2 = 0;i2 < encoded.length; i2++) {
    ciphertext[i2] = encoded[i2] ^ keyBytes[i2 % keyBytes.length];
  }
...
function getAmzDateComponents(now) {
  const dateStamp = now.toISOString().replace(/[-:]/g, "").slice(0, 8);
  const amzDate = dateStamp + "T" + now.toISOString().replace(/[-:]/g, "").slice(9, 15) + "Z";
  return { dateStamp, amzDate };
}
function sigV4Authorization(creds, region, method, canonicalUri, host, payload, dateStamp, amzDate) {
...
var onHttpTrigger = (runtime2, payload) => {
  ...
  runtime2.log(`
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
  runtime2.log("MemoryVault: Memory Writer — HTTP Trigger");
  runtime2.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  const inputData = decodeJson(payload.input);
  const agentId = inputData.agentId;
  const entryKey = inputData.entryKey;
  const entryData = inputData.entryData;
  ...
  runtime2.log(`[Step 1] Agent: ${agentId} | Key: ${entryKey}`);
  runtime2.log(`[Step 1] Entry data: ${JSON.stringify(entryData)}`);
  ...
  const timestamp = runtime2.now();
  const plaintext = JSON.stringify({
    ...entryData,
    timestamp
  });
  runtime2.log(`[Step 3] Plaintext prepared (${plaintext.length} chars, ts=${timestamp})`);
  runtime2.log("[Step 4] Encrypting data...");
  const encrypted = encryptData(plaintext, aesKey.value);
  runtime2.log(`[Step 4] Encrypted blob: ${encrypted.length} chars (base64)`);
  runtime2.log("[Step 5] Writing to S3...");
  ...
  for (let attempt = 1;attempt <= MAX_S3_RETRIES; attempt++) {
    try {
      runtime2.log(`[Step 5] S3 PUT attempt ${attempt}/${MAX_S3_RETRIES}: s3://${runtime2.config.s3Bucket}/${s3Key}`);
      const { dateStamp, amzDate } = getAmzDateComponents(timestamp);
      const authorization = sigV4Authorization(creds, runtime2.config.s3Region, "PUT", `/${s3Key}`, host, encrypted, dateStamp, amzDate);
      const payloadHash = hashHex(encrypted);
      httpClient.sendRequest(runtime2, (sendRequester) => {
      ...
      s3Success = true;
      runtime2.log(`[Step 5] S3 write succeeded on attempt ${attempt}`);
      break;
    } catch (e) {
      runtime2.log(`[Step 5] S3 write failed (attempt ${attempt}): ${e}`);
      if (attempt === MAX_S3_RETRIES) {
        runtime2.log("[Step 5] ⚠️ ALL S3 RETRIES EXHAUSTED — aborting commit");
      }
    }
  }
  if (!s3Success) {
  ...
  runtime2.log(`[Step 6] Entry hash (keccak256): ${entryHash}`);
  runtime2.log("[Step 7] Encoding report for MemoryRegistry...");
  const timestampMs2 = new Date(String(timestamp)).getTime();
  const timestampSecs = BigInt(Math.floor(timestampMs2 / 1000));
  const reportData = encodeAbiParameters(parseAbiParameters("string, string, bytes32, uint256"), [agentId, entryKey, entryHash, timestampSecs]);
  runtime2.log("[Step 8] Generating CRE report...");
  ...
  runtime2.log(`[Step 9] Writing report to MemoryRegistry: ${runtime2.config.memoryRegistryAddress}`);
  ...
  const writeResult = evmClient.writeReport(runtime2, {
    receiver: runtime2.config.memoryRegistryAddress,
    report: reportResponse,
    gasConfig: {
      gasLimit: runtime2.config.evms[0].gasLimit
    }
  }).result();
  if (writeResult.txStatus === TxStatus.SUCCESS) {
    const txHash = bytesToHex(writeResult.txHash || new Uint8Array(32));
    runtime2.log("[Step 10] ✅ Memory committed successfully");
    runtime2.log(`  Agent:    ${agentId}`);
    runtime2.log(`  Key:      ${entryKey}`);
    runtime2.log(`  Hash:     ${entryHash}`);
    runtime2.log(`  S3:       s3://${runtime2.config.s3Bucket}/${s3Key}`);
...
`);
  ...
  runtime2.log(`[Step 10] ❌ On-chain write failed: ${writeResult.txStatus}`);
  return JSON.stringify({
    status: "failed",
