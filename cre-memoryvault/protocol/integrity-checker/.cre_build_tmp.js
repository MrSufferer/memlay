...
function hexToBytes3(hex) {
  ...
  for (let i2 = 0;i2 < bytes.length; i2++) {
    bytes[i2] = parseInt(cleanHex.slice(i2 * 2, i2 * 2 + 2), 16);
  }
  return bytes;
}
function base64ToUint8Array(b64) {
  ...
  for (let i2 = 0;i2 < chars.length; i2++) {
    lookup2.set(chars[i2], i2);
  }
  const clean2 = b64.replace(/=/g, "");
  const byteLength2 = Math.floor(clean2.length * 3 / 4);
  const bytes = new Uint8Array(byteLength2);
  let j = 0;
  for (let i2 = 0;i2 < clean2.length; i2 += 4) {
    const a = lookup2.get(clean2[i2]) || 0;
    const b = lookup2.get(clean2[i2 + 1]) || 0;
    const c = lookup2.get(clean2[i2 + 2]) || 0;
    const d = lookup2.get(clean2[i2 + 3]) || 0;
    bytes[j++] = a << 2 | b >> 4;
    if (j < byteLength2)
      bytes[j++] = (b & 15) << 4 | c >> 2;
    if (j < byteLength2)
...
function decryptData(ciphertextB64, keyHex) {
  ...
  for (let i2 = 0;i2 < ciphertext.length; i2++) {
    plainBytes[i2] = ciphertext[i2] ^ keyBytes[i2 % keyBytes.length];
  }
  return new TextDecoder().decode(plainBytes);
}
function parseS3ListResponse(xml) {
  const keys = [];
...
function computeSigV4(accessKeyId, secretAccessKey, region, dateStamp, amzDate, method, canonicalUri, host, payload, queryString = "") {
...
var configSchema = exports_external.object({
  ...
  s3Bucket: exports_external.string(),
  s3Region: exports_external.string(),
  agentId: exports_external.string(),
  schedule: exports_external.string(),
  memoryRegistryAddress: exports_external.string(),
...
var onCronTrigger = (runtime2) => {
  runtime2.log(`
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
  runtime2.log("MemoryVault: Integrity Checker — Cron Trigger");
  runtime2.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  const agentId = runtime2.config.agentId;
  runtime2.log(`[Step 1] Integrity check for agent: ${agentId}`);
  runtime2.log("[Step 2] Fetching secrets...");
  const aesKey = runtime2.getSecret({ id: "AES_KEY" }).result();
  const awsAccessKeyId = runtime2.getSecret({ id: "AWS_ACCESS_KEY_ID" }).result();
  const awsSecretAccessKey = runtime2.getSecret({ id: "AWS_SECRET_ACCESS_KEY" }).result();
  const pushoverUserKey = runtime2.getSecret({ id: "PUSHOVER_USER_KEY" }).result();
  const pushoverApiToken = runtime2.getSecret({ id: "PUSHOVER_API_TOKEN" }).result();
  runtime2.log("[Step 3] Listing S3 objects...");
  ...
  const fetchS3List = (sendRequester, listUrl) => {
    ...
    if (!ok(resp)) {
      throw new Error(`S3 LIST failed with status: ${resp.statusCode}`);
    }
    return new TextDecoder().decode(resp.body);
  };
  let s3Keys = [];
  try {
    const listResponseBody = httpClient.sendRequest(runtime2, fetchS3List, consensusIdenticalAggregation())(`https://${host}/?prefix=${encodeURIComponent(prefix)}`).result();
    s3Keys = parseS3ListResponse(listResponseBody);
    runtime2.log(`[Step 3] Found ${s3Keys.length} S3 entries for agent ${agentId}`);
  } catch (e) {
    runtime2.log(`[Step 3] S3 LIST failed: ${e}`);
    runtime2.log("[Step 3] Continuing with empty list (simulation or S3 error)");
  }
  runtime2.log("[Step 4] Setting up EVM client...");
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: runtime2.config.evms[0].chainSelectorName,
    isTestnet: true
  });
  if (!network248) {
    return JSON.stringify({
      status: "failed",
      error: `Network not found for chain: ${runtime2.config.evms[0].chainSelectorName}`,
      agentId
    });
  }
  const evmClient = new cre.capabilities.EVMClient(network248.chainSelector.selector);
  runtime2.log("[Step 5] Reading, decrypting, and verifying entries...");
  const mismatches = [];
  let checkedCount = 0;
  let verifiedCount = 0;
  const fetchS3Object = (sendRequester, objectUrl, objectAuth) => {
    const resp = sendRequester.sendRequest({
      url: objectUrl,
      method: "GET",
      headers: {
    ...
    if (!ok(resp)) {
      throw new Error(`S3 GET failed with status: ${resp.statusCode}`);
    }
    return new TextDecoder().decode(resp.body);
  };
  for (const s3Key of s3Keys) {
    try {
      const getAuth = computeSigV4(awsAccessKeyId.value, awsSecretAccessKey.value, runtime2.config.s3Region, dateStamp, amzDate, "GET", `/${s3Key}`, host, "");
      const encryptedBlob = httpClient.sendRequest(runtime2, fetchS3Object, consensusIdenticalAggregation())(`https://${host}/${s3Key}`, getAuth).result();
      ...
      const callResult = evmClient.callContract(runtime2, {
        call: encodeCallMsg({
          from: zeroAddress,
          to: runtime2.config.memoryRegistryAddress,
          data: callData
        })
      }).result();
      ...
      checkedCount++;
      const isVerified = commitment.committedAt > 0n && commitment.entryHash === computedHash;
      if (isVerified) {
        verifiedCount++;
        runtime2.log(`  [${s3Key}] ✅ hash verified`);
      } else {
        runtime2.log(`  [${s3Key}] ❌ HASH MISMATCH — tampering detected!`);
        mismatches.push({
          key: s3Key,
          computedHash,
      ...
      runtime2.log(`  [${s3Key}] ⚠️ Error processing entry: ${e}`);
      checkedCount++;
      mismatches.push({
        key: s3Key,
        computedHash: "error",
  ...
  runtime2.log("[Step 6] Cross-checking on-chain agent commitments...");
  let onChainCount = 0n;
  try {
    ...
    runtime2.log(`[Step 6] On-chain commitments: ${onChainCount.toString()} | S3 entries: ${s3Keys.length}`);
    if (Number(onChainCount) > s3Keys.length) {
      runtime2.log(`[Step 6] ⚠️ WARNING: More on-chain commitments than S3 entries — possible S3 deletion`);
      mismatches.push({
    ...
    runtime2.log(`[Step 6] On-chain check failed: ${e}`);
  }
  if (mismatches.length > 0) {
    runtime2.log("[Step 7] \uD83D\uDEA8 TAMPERING DETECTED — sending Pushover alert...");
    const mismatchSummary = mismatches.slice(0, 5).map((m) => `• ${m.key}`).join(`
`);
    const moreText = mismatches.length > 5 ? `
...and ${mismatches.length - 5} more` : "";
    const postPushoverAlert = (sendRequester, pushoverConfig) => {
      const bodyBytes = new TextEncoder().encode(JSON.stringify(pushoverConfig));
    ...
    try {
      const result = httpClient.sendRequest(runtime2, postPushoverAlert, consensusIdenticalAggregation())({
...
${mismatchSummary}${moreText}`,
      ...
      runtime2.log(`[Step 7] Pushover alert sent (Status: ${result.statusCode})`);
    } catch (e) {
      runtime2.log(`[Step 7] ⚠️ Pushover alert failed: ${e}`);
    }
  } else {
    runtime2.log("[Step 7] ✅ No tampering detected — all entries verified");
  }
  runtime2.log(`
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
  runtime2.log(`Integrity check complete for agent: ${agentId}`);
  runtime2.log(`  Entries checked:  ${checkedCount}`);
  runtime2.log(`  Verified:         ${verifiedCount}`);
  runtime2.log(`  Mismatches:       ${mismatches.length}`);
  runtime2.log(`  On-chain count:   ${onChainCount.toString()}`);
  runtime2.log(`  Status:           ${mismatches.length > 0 ? "\uD83D\uDEA8 TAMPERING DETECTED" : "✅ ALL CLEAR"}`);
  runtime2.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
`);
  return JSON.stringify({
