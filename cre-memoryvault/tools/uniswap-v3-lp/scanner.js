...
var configSchema = exports_external.object({
  ...
  minTVL: exports_external.number().default(500000),
  maxAgeDays: exports_external.number().default(7)
});
var fetchOpportunities = (sendRequester, config) => {
  throw new Error("DEBUG config is: " + JSON.stringify(config) + " keys: " + Object.keys(config).join(","));
  const poolsUrl = `${config.apiUrl}/pools/clmm?minTVL=${config.minTVL}&maxAgeDays=${config.maxAgeDays}`;
  const poolsResult = sendRequester.sendRequest({
    request: {
      url: poolsUrl,
      method: "GET",
      multiHeaders: {
        "X-Api-Key": { values: ["{{.dataApiKey}}"] }
      }
    },
    vaultDonSecrets: [
      { key: "dataApiKey", owner: config.owner }
    ]
  }).result();
  if (!ok(poolsResult)) {
    throw new Error(`Failed to fetch pools: ${poolsResult.statusCode}`);
  }
  const { pools } = JSON.parse(new TextDecoder().decode(poolsResult.body));
  const opportunities = [];
  for (const pool of pools) {
    const trustUrl = `${config.apiUrl}/trust/${pool.token}`;
    const trustResult = sendRequester.sendRequest({
      request: {
        url: trustUrl,
        method: "GET",
        multiHeaders: {
    ...
    if (ok(trustResult) && trustResult.body) {
      const trustData = JSON.parse(new TextDecoder().decode(trustResult.body));
      trustSignals = {
        tokenSniffer: trustData.tokenSniffer,
        etherscan: trustData.etherscan,
        uncx: trustData.uncx,
        holders: trustData.holders
      };
    }
    opportunities.push({
...
var onCronTrigger = (runtime2) => {
  runtime2.log("Starting Uniswap V3 LP Scanner tool");
  const confHTTPClient = new ClientCapability;
  const result = confHTTPClient.sendRequest(runtime2, fetchOpportunities, consensusIdenticalAggregation())(runtime2.config).result();
  runtime2.log(`Scanner completed successfully. Found ${result.opportunities?.length || 0} opportunities.`);
  return result;
};
