...
var onCronTrigger = (runtime2) => {
  ...
  for (const poolId of config.activePositionPoolIds) {
    ...
    if (typeof pool.tvlChange4h === "number") {
      ...
      runtime2.log(`TVL crash trigger approximated/disabled for pool ${poolId}: ` + "tvlChange4h not provided by API.");
    }
    if (typeof pool.feeMultipleSinceEntry === "number") {
      if (pool.feeMultipleSinceEntry >= 2) {
        exitSignals.push({
      ...
      runtime2.log(`Profit target trigger approximated/disabled for pool ${poolId}: ` + "feeMultipleSinceEntry not provided by API.");
    }
  }
  const status = exitSignals.length > 0 ? "success" : "no_action";
  const response = {
    status,
    action: "monitor",
    toolId: "uniswap-v3-lp",
    data: {
      positionsChecked: checked2,
